...
## Tips
以上, 按着我的理解, 将Promises/A+规范基本完整的翻译了一遍, 除了一些Notes注解部分未翻译, 大家可以参考规范自行翻译.
...
class MPromise {
    ...
    FULFILLED_CALLBACK_LIST = [];  // 实例属性
    REJECTED_CALLBACK_LIST = [];
    _status = PENDING;
    constructor(fn) {
        // 初始状态为pending
        this.status = PENDING;  // 实例属性
        this.value = null;
        this.reason = null;
        try {
...
console.dir(test)
console.log(MPromise.FULFILLED_CALLBACK_LIST)
```
## Acknowledgments
